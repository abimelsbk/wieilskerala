...
const LegacyEvent = ({ year, location, description, imageUrl, isLeft = true }) => {
  return (
    <div className={`flex flex-col ${isLeft ? 'md:flex-row' : 'md:flex-row-reverse'} gap-6 md:gap-10 items-center mb-16`}>
      <div className="md:w-2/5">
        <div className="relative">
          <div className={`absolute inset-0 bg-gradient-to-r ${isLeft ? 'from-purple-400 to-indigo-400' : 'from-indigo-400 to-purple-400'} opacity-75 rounded-xl ${isLeft ? 'transform rotate-3' : 'transform -rotate-3'}`}></div>
          <img 
            src={imageUrl} 
            alt={`WIE ILS ${year}`} 
            className={`relative rounded-xl shadow-lg ${isLeft ? 'transform -rotate-3' : 'transform rotate-3'} transition-transform duration-500 hover:rotate-0`}
          />
        </div>
      </div>
      ...
      <div className="container mx-auto px-6">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <div className="inline-flex items-center gap-2 mb-4">
            ...
            <span className="text-purple-600 font-semibold tracking-wider uppercase text-sm">Our Legacy</span>
          </div>
          ...
          <p className="text-gray-600 leading-relaxed">
            From our humble beginnings to our current summit, we've been creating spaces for growth, connection, and transformation for women in engineering and leadership.
          </p>
        </div>
        ...
        <div className="mt-20">
          ...
          <div className="text-center mt-10">
            <h3 className="text-2xl font-bold text-gray-800 mb-4">Now in 2025, we're going even further.</h3>
            <p className="text-gray-600 max-w-2xl mx-auto">
              Join us in Kozhikode as we continue this journey of transformation, bringing together the brightest minds and most passionate hearts to shape the future of leadership.
            </p>
